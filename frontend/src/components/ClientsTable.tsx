...
export function ClientsTable() {
  ...
  // Map teamId -> teamName for sorting/accessor
  ...
  const columns = useMemo<ColumnDef<Client>[]>(() => [
    ...
    {
      // Provide a sortable value via accessorFn (team name)
      id: "team",
      header: "Team",
      accessorFn: (row) => teamNameById.get(row.teamId ?? "") ?? "",
      sortingFn: "alphanumeric", // or a custom fn if you want nulls last
      cell: ({ row }) => {
        ...
        return (
          <Dropdown
            ...
            showGenerator={(team, value) => !team.ip || value === team.id}
